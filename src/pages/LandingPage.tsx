...
export default function LandingPage() {
  ...
  return (
    <div className="flex flex-col flex-1 bg-[#F2F2EC] dark:bg-slate-950 transition-colors duration-300">
      ...
      <section className="flex-1 flex flex-col items-center justify-center text-center px-4 sm:px-6 lg:px-8 py-24 border-b-[6px] border-black dark:border-slate-800 overflow-hidden relative">
        ...
        <div className="max-w-4xl relative z-10 bg-[#F2F2EC] dark:bg-slate-900/80 backdrop-blur-sm p-8 border-4 border-black dark:border-slate-700 shadow-[12px_12px_0_0_#000] dark:shadow-[12px_12px_0_0_#1e293b] transition-all duration-300">
          <h1 className="text-6xl sm:text-8xl md:text-[100px] font-black text-black dark:text-white tracking-tighter leading-[0.9] mb-8 text-left break-words">
            ...
            <span className="text-[#FFD700] dark:text-indigo-500" style={{ textShadow: 'none' }}>{t('hero.line2').toLocaleUpperCase(language === 'tr' ? 'tr-TR' : 'en-US')}</span>
          </h1>
          <p className="text-xl sm:text-3xl font-bold tracking-wide text-black dark:text-slate-300 mb-12 leading-relaxed text-left border-l-[6px] border-black dark:border-indigo-500 pl-6">
            {t('hero.desc').toLocaleUpperCase(language === 'tr' ? 'tr-TR' : 'en-US')}
          </p>
          <div className="flex flex-col sm:flex-row gap-6 justify-start">
            <Link 
              to="/harita" 
              className="group inline-flex items-center justify-center px-8 py-5 text-sm font-black tracking-[0.2em] text-[#FFD700] dark:text-white bg-black dark:bg-indigo-600 border-4 border-black dark:border-indigo-600 hover:bg-white dark:hover:bg-indigo-500 hover:text-black dark:hover:text-white hover:shadow-[8px_8px_0_0_#000] dark:hover:shadow-[8px_8px_0_0_#312e81] transition-all"
            >
      ...
      {/* Features Section */}
      <section id="features" className="py-24 bg-[#FFD700] dark:bg-slate-900 border-b-[6px] border-black dark:border-slate-800 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-16 border-b-8 border-black dark:border-indigo-500 pb-8 inline-block transition-colors">
            <h2 className="text-5xl md:text-7xl font-black tracking-tighter text-black dark:text-white leading-none">{t('features.title').toLocaleUpperCase(language === 'tr' ? 'tr-TR' : 'en-US')}</h2>
            <p className="mt-4 text-xl font-bold tracking-widest text-[#1A1A1A] dark:text-slate-400">{t('features.subtitle').toLocaleUpperCase(language === 'tr' ? 'tr-TR' : 'en-US')}</p>
          </div>
      ...
      <section className="py-24 bg-black dark:bg-slate-950 text-[#F2F2EC] dark:text-slate-200 text-center border-t-[6px] border-black dark:border-slate-800 transition-colors duration-300">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <Link 
            to="/harita" 
            className="inline-flex items-center justify-center px-12 py-6 text-xl font-black tracking-[0.3em] text-black dark:text-white bg-[#FFD700] dark:bg-indigo-600 hover:bg-white dark:hover:bg-indigo-500 hover:scale-105 border-4 border-[#FFD700] dark:border-indigo-600 hover:border-white transition-all transform shadow-[12px_12px_0_0_#FFF] dark:shadow-[12px_12px_0_0_#312e81]"
          >
...
function FeatureCard({ icon, title, description, language, to }: { icon: React.ReactNode, title: string, description: string, language: string, to: string }) {
  return (
    <Link to={to} className="block p-8 bg-[#F2F2EC] dark:bg-slate-800 border-4 border-black dark:border-slate-700 shadow-[8px_8px_0_0_#000] dark:shadow-[8px_8px_0_0_#0f172a] hover:-translate-y-2 hover:shadow-[16px_16px_0_0_#000] dark:hover:shadow-[16px_16px_0_0_#0f172a] transition-all group">
      <div className="w-20 h-20 border-4 border-black dark:border-slate-600 bg-white dark:bg-slate-700 flex items-center justify-center mb-6 group-hover:bg-[#FFD700] dark:group-hover:bg-indigo-600 transition-colors">
        {icon}
      </div>
      ...
      <p className="text-base font-bold text-gray-800 dark:text-slate-300 leading-relaxed">{description.toLocaleUpperCase(language === 'tr' ? 'tr-TR' : 'en-US')}</p>
    </Link>
  );
}
